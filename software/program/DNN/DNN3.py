...
def init_params(options):
    ...
    params['U2'] = (1/numpy.sqrt(options['dim1']) * ( 2 * numpy.random.rand(options['dim1'],options['dim2']) - 1)).astype(config.floatX)
    params['b2'] = numpy.zeros((options['dim2'],)).astype(config.floatX)
    
    params['U3'] = (1/numpy.sqrt(options['dim2']) * ( 2 * numpy.random.rand(options['dim2'],options['dim_output']) - 1)).astype(config.floatX)
    params['b3'] = numpy.zeros((options['dim_output'],)).astype(config.floatX)
    return params
...
def build_model(tparams, options):
    ...
    proj2 = tensor.tanh(tensor.dot(proj1, tparams['U2']) + tparams['b2'])
    proj3 = tensor.nnet.softmax(tensor.dot(proj2, tparams['U3']) + tparams['b3'])
    cost =  tensor.sum(y * tensor.log(y/proj3))
    return  x, y, cost, proj3
...
def train(
    ...
    for k, p in tparams.iteritems():
        if k  in ['U2','b2']: 
            ...
            grads[id]=grads[id]/numpy.sqrt(model_options['dim2'])
        #elif k in ['U1','b1']:
        #    grads[id]=grads[id]/numpy.sqrt(model_options['dim_input'])
        id += 1
    ...
    try:
        ...
        layer3 = numpy.abs(numpy.tanh(numpy.dot(  numpy.tanh(numpy.dot(  numpy.tanh(params['U1'] + params['b1'])  ,params['U2']  ) + params['b2'] ), params['U3']) + params['b3'] ))
        
        for id in range(dim1):
            top20_id = numpy.argsort(layer1[:,id])[::-1][:20]
            fout_a.write('1 '+str(id)+' '+' '.join([id2word[w] for w in top20_id]) + '\n')
        for id in range(dim2):
            top20_id = numpy.argsort(layer2[:,id])[::-1][:20]
            fout_a.write('2 '+str(id)+' '+' '.join([id2word[w] for w in top20_id]) + '\n')
        for id in range(dim_output):
            top20_id = numpy.argsort(layer3[:,id])[::-1][:20]
            fout_a.write('3 '+str(id)+' '+' '.join([id2word[w] for w in top20_id]) + '\n')
        fout_a.close()
...
if __name__ == '__main__':
    ...
    train(
    dim1= 3*int(sys.argv[1]), 
    dim2= 2*int(sys.argv[1]),
