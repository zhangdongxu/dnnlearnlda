...
/*
 *save the final gamma parameters of the final datset
 *
 */

void save_gamma_final(char* filename, double** gamma, int num_docs, int num_topics, corpus* corpus)
{
    FILE* fileptr;
    int d, k;
    fileptr = fopen(filename, "w");
    ...
    {
        // printf("%s ", corpus->docs[d].id);
        fprintf(fileptr, "%s ", corpus->docs[d].id);
        fprintf(fileptr, "%5.10f", gamma[d][0]);
        for (k = 1; k < num_topics; k++)
        {
            fprintf(fileptr, " %5.10f", gamma[d][k]);
...
void run_em(char* start, char* directory, corpus* corpus)
{
   ...
   //     if((i % 1) == 0)
        if ((i % LAG) == 0)
        {
            ...
            sprintf(filename,"%s/%03d.gamma.labeld",directory, i);
            save_gamma_final(filename, var_gamma, corpus->num_docs, model->num_topics,corpus);
        }
    }
    ...
    save_gamma(filename, var_gamma, corpus->num_docs, model->num_topics);
    sprintf(filename,"%s/final.gamma.labeld",directory);
    save_gamma_final(filename, var_gamma, corpus->num_docs, model->num_topics,corpus);
